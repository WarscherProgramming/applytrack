...
`auth`, `users`, `companies`, `applications`, `recruiters`, `interviews`,
...
- **`features/auth/`** - login/register API wrappers, token types, AuthProvider,
  protected-route wrapper, and account-settings card.
...
- **Alembic** migrations live in `backend/migrations/versions/`, numbered
  `0001`…`0014`, each with explicit `upgrade()`/`downgrade()` and a linear
...
`notifications`, `calendar_connections`, `calendar_sync_events`, `tasks`, `users`, `auth_refresh_tokens`.
...
with expiry/revocation metadata. Existing CRM/domain tables are not user-owned
yet; that ownership-scoping pass is the next production-readiness step.
...
Authentication establishes identity without changing existing domain ownership
rules yet:
...
- **Scope boundary:** the frontend app shell is protected, and account endpoints
  require authentication. Existing CRM feature routers are intentionally not
  globally protected in M29 because their rows do not yet carry `user_id`; route
  protection and repository scoping should happen together in the next pass to
  avoid misleading multi-user isolation.
