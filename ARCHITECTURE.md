...
`opportunity_discovery`, `daily_briefing`, `calendar_integration`.
...
- **Alembic** migrations live in `backend/migrations/versions/`, numbered
  `0001`…`0012`, each with explicit `upgrade()`/`downgrade()` and a linear
...
`notifications`, `calendar_connections`, `calendar_sync_events`.
...
- **Current status:** 436 passing on the last full committed run; M27 adds 4
